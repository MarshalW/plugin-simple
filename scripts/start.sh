...
# 插入的逻辑开始
SRC_DIR="/simple-app/node_modules/@marshal"
DST_BASE="/app/nocobase/node_modules"
DST_DIR="${DST_BASE}/@marshal"

if [ ! -d "$DST_DIR" ]; then
  echo "Directory $DST_DIR not found, copying..."
  cp -r "$SRC_DIR" "$DST_BASE"
else
  echo "Directory $DST_DIR already exists, skip copy."
fi
# 插入的逻辑结束
